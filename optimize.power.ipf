...
macro Optimize_Power()
	...
	variable /g max_power_0=7, min_power_0=2
...
// parse igor system variables to extract the basename used by triggersync file naming scheme
	...
	do //do1
		...
		if ( WaveExists(root:uncage_power_wave))
		// uncage_power_wave does not exist on the first iteration of the do loop, therefore this block only executes after the first loop
		// update the input and output filenames then read response data
		...
		// estimate baseline current before uncaging
		K0 =  mean(ach_1,0,uncgpnt)
		// estimate peak as largest deviation from baseline
		Loess/pass=1 /N=(2^7) /DEST=temp srcWave=$traceunc
		temp = (abs(temp - K0))
		wavestats /q temp
		peak_time = v_maxloc
		K1 = $traceunc(v_maxloc)-k0
		// set decay amplitude to hard coded constant
		K2 = td

		// unclear what this wavestats call does
		...
		// read power from triggersync configuration file
		last_power = read_power_from_prm(prm_file_name)
		...
		// if the last power satisfies the convergence criteria with respect to the target power, then write output files and break
		...
		if(temp_var == 2)
		// user selected to write output files using last power
			duplicate /o uncage_power_lut temp
		...
		if(temp_var == 3)
		//user selected to write output files using next calculated power
			duplicate /o uncage_power_lut temp
			temp = temp -	uncage_power_lut(next_power)/1.41421
			temp = abs(temp)
			wavestats /q temp
...
// calculate next power using geometric series
function next_power_geo(this_response, target_response, last_power)
	...
	if(this_response >= target_response)
		...
		temp = abs(temp)
		wavestats /q temp
		min_power = min(min_power,uncage_volt_lut[x2pnt(temp, v_minloc)])
		max_power = last_power
		return 0
	endif
end

// calculate next power using binary search
...
// calculate next power using the square law relation for a two photon process R = const*I^2
...
// calculate next power by finite difference
...
// support function to read trigger sync configuration files and write with new power
...
// support file to read uncaging power in from triggersync configuration file
...
// support macro for correcting the uncaging position offset in mark points
...
// support function for creating a lookup table for laser power vs pockels voltage
...
// genrate custon "macros" menu in igor
menu "macros"
	"uncage_pos_cor"
	"fit_power"
	"optimize_power"

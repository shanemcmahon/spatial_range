// uncaging.analysis.2 performs fits with constraints on some parameters
// box constraints caused the algorithm to perform poorly, therefore "soft" constraints were implemented using penalized least square fitting on reparametrized function
...
Function DiffTwoExp2(w,t) : FitFunc
    ...
    if(w[1]^2 > w[6])
      if((t-(w[1]^2+w[5])) > -0.01 )
			// w[1]^2 in the model specifies the time for the response onset, w[5] specifies an upper bound for the onset time
			// here we provide an exponentially increasing penalty for violating the upper bound
...
duplicate /o /r=[0,8] w_amplitude_0 WTemp2
